		...
		int maxIterationNum, double convergenceThreshold)
{
  ...
  LabelImage3::Pointer canvas = readImage<LabelImage3>(labelImageNames[0]);
  for (int i = 0; i < n; ++i) {
    std::cerr << "Generating texton patches for volume " << i << "... ";
    FloatImage3::Pointer valImage = 
      readImage<FloatImage3>(valImageNames[i]);
    LabelImage3::Pointer labelImage = 
      readImage<LabelImage3>(labelImageNames[i]);
    PointMap3 rmap;
...
int main (int argc, char* argv[])
{
  if (argc < 6) {
    std::cerr << "Usage: " << argv[0] 
	      << " {-v valueImageVolumeName}"
	      << " {-l labelImageVolumeName}"
